...
function createMcpServer() {
  ...
  return {
    tools: [
      ...
      { name: "bridge", description: "Generate a CCTP V2 depositForBurnWithHook transaction", inputSchema: { type: "object", properties: { chain: { type: "string" }, amount: { type: "string" }, destinationDomain: { type: "number" }, mintRecipient: { type: "string" }, burnToken: { type: "string" }, destinationCaller: { type: "string" }, maxFee: { type: "string" }, minFinalityThreshold: { type: "number" }, hookData: { type: "string" } }, required: ["chain", "amount", "destinationDomain", "mintRecipient", "burnToken"] } },
