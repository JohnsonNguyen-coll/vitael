  ...
  return (
    <div>
        ...
        <div className="flex gap-1.5">
          {[0.5, 1.0].map(v => (
            <button key={v} onClick={() => setSlippage(v)}
              className={`px-2.5 py-1 rounded-lg text-xs font-semibold transition ${slippage === v ? "bg-[#00F5FF] text-[#0A1428]" : "bg-white/5 text-[#8E9FB8] hover:bg-white/10"}`}>
              {v}% slip
            </button>
          ))}
        </div>
      ...
      <div className="flex justify-center my-1">
        <div className="w-9 h-9 rounded-full bg-[#0A1428] border border-white/10 flex items-center justify-center text-[#8E9FB8]">
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <span className="text-[#8E9FB8] font-bold">/</span>
      ...
      {poolInfo ? (
        <div className="bg-white/2 rounded-xl p-3 mb-4 text-xs">
          <div className="flex justify-between mb-3">
            <span className="text-[#8E9FB8]">Your LP balance</span>
          ...
          <div className="flex gap-2">
            {[25, 50, 75, 100].map(p => (
              <button key={p} onClick={() => handlePct(p)}
                className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition ${pct === p ? "bg-[#00F5FF] text-[#0A1428]" : "bg-white/5 text-[#8E9FB8] hover:bg-white/10"}`}>
                {p}%
              </button>
            ))}
          </div>
        ...
        <div className="bg-white/2 rounded-xl p-3 mb-4 text-xs text-[#8E9FB8]">
      ...
      {lpAmount && parseFloat(lpAmount) > 0 && (
        <div className="bg-white/2 rounded-xl p-3 mb-4 text-xs space-y-1.5">
          <p className="text-[#8E9FB8] uppercase tracking-wider font-semibold mb-1">You will receive</p>
          <div className="flex justify-between">
      ...
      {!isConnected ? (
        ...
        <button onClick={() => { reset(); setLpAmount(""); setPct(100); }}
          className="w-full bg-white/5 border border-white/10 text-white font-semibold py-3.5 rounded-xl hover:bg-white/10 transition">Remove More</button>
      ) : (
        <button onClick={() => removeLiquidity(tokenA, tokenB, lpAmount, 0.5)}
          disabled={busy || !lpAmount || parseFloat(lpAmount) <= 0 || !poolInfo}
          className="w-full bg-red-400/80 text-white font-bold py-3.5 rounded-xl hover:bg-red-400 transition disabled:opacity-40 flex items-center justify-center gap-2">
    ...
    <div className="relative min-h-screen text-white font-sans">
      ...
      <main className="relative z-10 max-w-5xl mx-auto px-6 py-12">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
          ...
          <h1 className="text-4xl font-extrabold text-white">Liquidity</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 items-start">
              ...
              {([
                ...
                <button key={t.id} onClick={() => setTab(t.id)}
                  className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold transition ${
                    tab === t.id ? "bg-[#00F5FF] text-[#0A1428] shadow-lg" : "text-[#8E9FB8] hover:text-white"
                  }`}>
          ...
          <div className="lg:col-span-2 space-y-5">
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 }}
              ...
              <p className="text-xs uppercase tracking-wider text-[#8E9FB8] mb-4">Available Pools</p>
              <div className="space-y-3">
                {[{ a: "USDC", b: "EURC", fee: "0.3%" }, { a: "USDC", b: "cirBTC", fee: "0.3%" }].map(p => (
                  <div key={`${p.a}-${p.b}`} className="flex items-center justify-between py-2.5 border-b border-white/5 last:border-0">
                    ...
                    <span className="text-xs text-[#00F5FF] font-semibold bg-[#00F5FF]/10 px-2 py-0.5 rounded-full">{p.fee} fee</span>
                  </div>
                ))}
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}
              className="glass-panel rounded-3xl p-5">
              <p className="text-xs uppercase tracking-wider text-[#00F5FF] font-bold mb-3">How it works</p>
              <div className="space-y-3">
                {[
                  ...
                  <div key={s.n} className="flex gap-3">
                    <div className="w-6 h-6 rounded-full bg-[#00F5FF]/10 border border-[#00F5FF]/20 flex items-center justify-center text-xs font-bold text-[#00F5FF] flex-shrink-0 mt-0.5">{s.n}</div>
                    <div><p className="text-sm font-bold text-white">{s.t}</p><p className="text-xs text-[#8E9FB8] mt-0.5">{s.d}</p></div>
                  </div>
                ))}
              </div>
            </motion.div>
