...
// A Latch token authenticates to the proxy as a Bearer token. Explicitly clear
// apiKey so the Anthropic SDK does not also send X-Api-Key with the lat_ token.
// Without a proxy, keep the native Anthropic X-Api-Key authentication flow.
  ...
  for (let step = 0; step < 5; step++) {
    const response = await client.messages.create({
      ...
      system:
        ...
        "\n\nIMPORTANT FOR AMOUNTS: Use the token's smallest unit unless a tool explicitly requests human-readable units. BRIDGE IS THE EXCEPTION: bridge.amount must be the exact human-readable amount requested by the user (for example, pass \"0.1\" for 0.1 USDC), without subtracting fees. bridge.maxFee remains atomic units from quoteBridge. For other atomic arguments, 1 USDC = 1000000, 1 EURC = 1000000, and 1 cirBTC = 100000000." +
        "\n\nIMPORTANT FOR LP POSITIONS: Always call getLiquidityPosition when the user asks whether they have LP, how much LP they own, or wants to remove liquidity. Treat lpBalanceRaw > 0 as an active position. Show displayBalance to the user, not the canonical 18-decimal lpBalance. Explain the pool share and redeemable underlying amounts instead of describing it as 'almost no LP'. Use lpBalanceRaw as the liquidity argument when removing LP." +
