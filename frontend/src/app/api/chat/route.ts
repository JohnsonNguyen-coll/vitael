  ...
  for (let step = 0; step < 5; step++) {
    const response = await client.messages.create({
      ...
      system:
        ...
        "\n\nFor bridge.amount, pass the exact human-readable USDC amount requested by the user (for example, \"0.1\"), never an atomic conversion and never an amount with fees pre-subtracted. bridge.maxFee remains in atomic units from quoteBridge." +
