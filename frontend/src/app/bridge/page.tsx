  ...
  return (
    <div className="relative flex-1">
      ...
      <button onClick={() => setOpen(!open)}
        ...
        <ChevronDown className={`w-4 h-4 text-[#8991AF] transition-transform duration-200 ${open ? "rotate-180" : ""}`} />
      </button>
      <AnimatePresence>
        {open && (
        ...
        <div key={name} className="flex items-center flex-1">
          <div className={`flex items-center gap-1.5 flex-1 ${i < STEP_NAMES.length - 1 ? "" : ""}`}>
            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 transition-all duration-500 ${
              i < active  ? "bg-emerald-400 text-[#0D0E1E]"
              : i === active ? "bg-[#A998FF] text-[#0D0E1E] ring-2 ring-[#A998FF]/30"
              : "bg-white/10 text-[#8991AF]"
            }`}>
              {i < active ? "✓" : i + 1}
            </div>
            <span className={`text-xs hidden sm:block ${i === active ? "text-white" : "text-[#8991AF]"}`}>{name}</span>
          </div>
    ...
    <PageLayout variant="app">
      <main className="app-page relative z-10 max-w-5xl mx-auto px-6 py-12">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
          ...
          <h1 className="app-page-title text-5xl text-white">Bridge</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 items-start">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
            ...
            <div className="absolute -bottom-12 -left-12 w-48 h-48 bg-[#7968E8]/8 rounded-full blur-3xl pointer-events-none overflow-hidden" />
            ...
            <div className="flex items-end gap-3 mb-5">
              ...
              <button onClick={swapChains} disabled={busy}
                className="mb-1 w-10 h-10 flex-shrink-0 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-[#A998FF] hover:bg-[#A998FF]/10 hover:border-[#A998FF]/30 transition duration-200 disabled:opacity-40">
            ...
            {switchingChain && (
              <div className="bg-[#A998FF]/5 border border-[#A998FF]/15 rounded-xl px-4 py-3 mb-4 flex items-center gap-3">
                <div className="w-4 h-4 border-2 border-[#A998FF]/30 border-t-[#A998FF] rounded-full animate-spin flex-shrink-0" />
                <p className="text-sm text-[#A998FF]">Switching wallet to {fromChain.name}...</p>
              </div>
            )}
            ...
            {/* Token — USDC only */}
            <div className="mb-4">
              <p className="text-xs uppercase tracking-wider text-[#8991AF] mb-2">Token</p>
            ...
            {/* Amount */}
            <div className="mb-5">
              <p className="text-xs uppercase tracking-wider text-[#8991AF] mb-2">Amount (USDC)</p>
              <div className="relative bg-black/30 border border-white/5 focus-within:border-[#A998FF] rounded-2xl p-4 transition duration-200">
                ...
                <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[#A998FF] font-bold text-sm">USDC</span>
              </div>
            </div>
            ...
            {numAmt > 0 && !sameChain && !busy && !isDone && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                ...
                <div className="flex justify-between">
                  <span className="text-[#8991AF]">You send</span>
                  <span className="text-white font-semibold">{numAmt} USDC</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#8991AF]">Protocol</span>
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-[#A998FF] font-semibold">2 (Approve + Burn)</span>
                </div>
              </motion.div>
            )}
            ...
            {busy && (
              <div className="bg-[#A998FF]/5 border border-[#A998FF]/15 rounded-xl px-4 py-3 mb-5 flex items-center gap-3">
                <div className="w-4 h-4 border-2 border-[#A998FF]/30 border-t-[#A998FF] rounded-full animate-spin flex-shrink-0" />
                <p className="text-sm text-[#A998FF]">{state.stepLabel}</p>
              </div>
            )}
              ...
              <div className="space-y-2 mb-5">
                {state.approveTx && (
                  <a href={`${state.srcExplorer}${state.approveTx}`}
                    target="_blank" className="flex items-center gap-2 text-xs text-[#8991AF] hover:text-[#A998FF] transition">
                ...
                {state.burnTx && (
                  <a href={`${state.srcExplorer}${state.burnTx}`} target="_blank"
                    className="flex items-center gap-2 text-xs text-[#8991AF] hover:text-[#A998FF] transition">
                    <ExternalLink className="w-3.5 h-3.5" />
            ...
            {!isConnected ? (
              ...
              <button onClick={() => { reset(); setAmount(""); }}
                className="app-button app-button-secondary w-full bg-white/5 border border-white/10 text-white font-semibold py-3.5 rounded-xl hover:bg-white/10 transition duration-300">
                Bridge Again
              </button>
            ) : (
              <button onClick={execute} disabled={busy || !amount || numAmt <= 0 || sameChain}
                className="app-button app-button-primary w-full bg-[#A998FF] text-[#0D0E1E] font-bold py-3.5 rounded-xl hover:bg-white transition duration-300 disabled:opacity-40 flex items-center justify-center gap-2">
                {busy
                  ? <><div className="w-5 h-5 border-2 border-[#0D0E1E]/30 border-t-[#0D0E1E] rounded-full animate-spin" />{switchingChain ? "Switching network..." : state.stepLabel}</>
                  : `Bridge ${numAmt || ""} USDC → ${toChain.name}`}
              </button>
            )}

            <p className="text-center text-xs text-[#8991AF] mt-3">
              Powered by{" "}
              <a href="https://developers.circle.com/cctp" target="_blank" className="text-[#A998FF] hover:underline">
                Circle CCTP V2
              </a>
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 }}
              ...
              <p className="text-xs uppercase tracking-wider text-[#A998FF] font-bold mb-4">How it works</p>
                ...
                {[
                  ...
                  <div key={s.n} className="flex gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[#8991AF] mt-0.5 leading-relaxed">{s.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}
              className="glass-panel rounded-3xl p-5">
              <p className="text-xs uppercase tracking-wider text-[#8991AF] mb-4">Supported Chains</p>
              <div className="space-y-2">
                {CHAINS.map(c => (
                  <div key={c.id} className="flex items-center gap-2 py-1.5">
                    <ChainIcon chainId={c.id} size={20} />
                    <span className="text-xs text-white">{c.name}</span>
                    <span className="text-xs text-[#8991AF] ml-auto">Domain {c.domain}</span>
                  </div>
                ))}
              </div>
